...
import { useFetchInfiniteProjectMyList } from './hooks/project/useFetchProject';

const MypageProjectsMain = () => {
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="flex flex-col items-start justify-between border-gray-100 pb-6 sm:flex-row sm:items-center">
        <HeaderTitleBox
          title="프로젝트"
          content={`${allProjects.length}개의 프로젝트가 등록되어 있습니다`}
          icon={FolderOpen}
        />
        <Button
        ...
        {allProjects.length === 0 ? (
          ...
          <>
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              {allProjects.map(
                ({ id, title, description, created_at, slug }) => (
                  <article
                    key={id}
...
export default MypageProjectsMain;
